...
void setup(void) 
{
  ...
  //Typically the Gain and sample data rate should be set in setup as these typically don't change for a project, but you can change them any time!
    adc.setGain(ADS1115_REG_CONFIG_PGA_2_048V);
...
void loop(void) {
  ...
  float time = 0;
    ...
    Serial.print("Setting mux to single ended mode between AIN0 and GND @ 860SPS...");
      adc.setMux(ADS1115_REG_CONFIG_MUX_SINGLE_0); //Set single ended mode between AIN0 and GND
      adc.setSampleRate(ADS1115_REG_CONFIG_DR_860SPS); //Not truely required as this parameter was already configured in SETUP
    ...
    Serial.println("triggering conversion...");
      ...
      voltage = ((rawResult * 1.0 / 32768) * 2048) / 1000;  // (rawResult * 1.0 / ADS1115_REG_FACTOR) * voltageRange;
      endtime = micros();
      time = endtime - starttime;
      freq = 1/((endtime - starttime) * 0.000001);
    Serial.print("raw: "); Serial.print(rawResult); Serial.print(", Voltage: "); Serial.print(voltage); Serial.print(", freq read "); Serial.print(freq); Serial.print(", time "); Serial.print(time); Serial.print("us");
    Serial.println("=======================");
